...
dbExistsTable(con, c("Zues", "rundb_pluginresult"))
dbExistsTable(con, c("Zues", "rundb_experimentanalysissettings"))

myTable <- dbReadTable(con, c("Zues","rundb_pluginresult"))
...
myTable2 <- dbReadTable(con, c("Zues","rundb_experimentanalysissettings"))
...
write.table(x = tableOfFun.filtered, file = "/home/kevhu/data/20170728ZuesNew.tsv", sep = "\t", row.names = FALSE)
