...
cnCalls_1 <- read.table("/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-138-MG_cho_20210621_354_343/cnMatrix_gene.txt",
                        header = TRUE, stringsAsFactors = FALSE, sep = "\t")
cnCalls_2 <- read.table("/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-141-MG_cho_202106_3TS_356_349/cnMatrix_gene.txt",
                        header = TRUE, stringsAsFactors = FALSE, sep = "\t")
...
# write.table(tcDf, "/mnt/DATA5/tmp/kev/misc/20211101hgscTcDf_median.txt", sep = "\t",
#               quote = FALSE, row.names = FALSE, col.names = TRUE)
# 
...
# save(armRes, file = "/mnt/DATA5/tmp/kev/misc/20210828tcgaArmRes10Mb.Robj")
load("/mnt/DATA5/tmp/kev/misc/20210828tcgaArmRes10Mb.Robj")
...
# new - fixed
all_segRes$seg.mean <- 2^all_segRes$seg.mean
for (i in which(!is.na(matchingTc))) {
  tmpVec <- all_segRes$seg.mean[i]
  tmpVec[tmpVec > 1] <- tmpVec[tmpVec > 1] / matchingTc[i]
  tmpVec[tmpVec < 1] <- tmpVec[tmpVec < 1] * matchingTc[i]
  all_segRes$seg.mean[i] <- tmpVec
}

all_segRes$seg.mean <- log2(all_segRes$seg.mean)

# all_segRes$seg.mean[which(!is.na(matchingTc))] <- all_segRes$seg.mean[which(!is.na(matchingTc))]/matchingTc[which(!is.na(matchingTc))] - wrong less accurate
...
###
###
### all seg results - only for xiaoman data
segZfilt_all <- segZscoresFilt_zeroOut(all_segRes)
tmpSeg_all <- cbind(segZfilt_all[,1:4], NA, segZfilt_all[,5])
colnames(tmpSeg_all) <- c("sampleID","chrom", "start.pos","end.pos", "n.probes", "mean")
tmpSeg_all$string <- paste0(tmpSeg_all$sampleID, tmpSeg_all$chrom, tmpSeg_all$start.pos, tmpSeg_all$end.pos)
#tmpSeg_all <- tmpSeg_all[-which(duplicated(tmpSeg_all$string)),]
tmpSeg_all <- tmpSeg_all[order(tmpSeg_all$chrom, tmpSeg_all$start.pos),]

tmpSeg_all$mean[which(abs(tmpSeg_all$mean) < 0.2)] <- 0
tmpSeg_all$mean[which(tmpSeg_all$mean > log2(1/2) & tmpSeg_all$mean < log2(3/2))] <- 0

#allFga <- fgaCalculator_amp(tmpSeg_all)
allFga <- fgaCalculator_genes_amp(tmpSeg_all)
armRes_all <- separateSegments_mV4(tmpSeg_all)
# all_arm <- do.call(rbind, armRes_all[[1]])
# all_cna <- do.call(rbind, armRes_all[[2]])
# all_count <- do.call(rbind, armRes_all[[3]])

annoTabAge <- readxl::read_xlsx("/mnt/DATA5/tmp/kev/misc/20210823choLabAging.xlsx")
annoTabAge$ID <- str_remove_all(annoTabAge$ID, " ")
annoTabAge$ID <- str_remove_all(annoTabAge$ID, "O")
annoTabAge$ID <- tolower(annoTabAge$ID)
allFga$X1[which(allFga$X1 == "14656peritnealmt")] <- "14656peritonealmt"
rownames(all_count)[which(rownames(all_count) == "14656peritnealmt")] <- "14656peritonealmt"

agingFga <- allFga[which(allFga$X1 %in% annoTabAge$ID),]
agingFga$group <- annoTabAge$Study[match(agingFga$X1, annoTabAge$ID)]
t.test(data = agingFga, X2 ~ group)

pdf("/mnt/DATA5/tmp/kev/misc/20210918choBoxplotAging.pdf")
ggplot(data = agingFga, aes(x = group, y = X2)) + geom_boxplot() + 
  geom_signif(comparisons = list(c("Aging (control)","Aging (aged)")), test = "t.test") +
  ggtitle("Boxplot Fga Aging") + 
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
                     plot.title = element_text(hjust = 0.5)) +
  ylab("Fraction of genome altered") + xlab("Group") 
dev.off()

pregSampsHeat <- c("5293lt", "12596rt", "12641lt", "12214rt", "12639rt", "12779rt", "12127lt", "12342lt", "15035rt", "12240rt", "12461lt", "12461rt")
annoTabPreg1 <- readxl::read_xlsx("/mnt/DATA5/tmp/kev/misc/20210823choLabPregNf.xlsx")
annoTabPreg2 <- readxl::read_xlsx("/mnt/DATA5/tmp/kev/misc/20210823choLabPreReg.xlsx")
annoTabPreg <- rbind(annoTabPreg1, annoTabPreg2)
annoTabPreg$ID <- str_remove_all(annoTabPreg$ID, " ")
annoTabPreg$ID <- str_remove_all(annoTabPreg$ID, "O")
annoTabPreg$ID <- tolower(annoTabPreg$ID)
annoTabPreg <- annoTabPreg[which(annoTabPreg$ID %in% pregSampsHeat),]

pregFga <- allFga[which(allFga$X1 %in% annoTabPreg$ID),]
pregFga$group <- annoTabPreg$Study[match(pregFga$X1, annoTabPreg$ID)]
pregFga$group1 <- c("Multiparity", "Multiparity", "Nullparity", "Multiparity", "Multiparity", "Nullparity",
                    "Nullparity", "Multiparity", "Multiparity", "Multiparity", "Nullparity", "Nullparity")

pregFga$group2 <- c("BPRN", "BPRN", "BPRN", "BPRN", "BPRN", "BPRN",
                    "BPRN fl/+",  "BPRN fl/+",  "BPRN fl/+",  "BPRN fl/+",  "BPRN fl/+",  "BPRN fl/+")

t.test(data = pregFga, X2 ~ group1)
t.test(data = pregFga, X2 ~ group2)

pdf("/mnt/DATA5/tmp/kev/misc/20210918choBoxplotPregPar.pdf")
ggplot(data = pregFga, aes(x = group1, y = X2)) + geom_boxplot() + 
  geom_signif(comparisons = list(c("Nullparity","Multiparity")), test = "t.test") +
  ggtitle("Boxplot Fga Pregnancy (Parity)") + 
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
                     plot.title = element_text(hjust = 0.5)) +
  ylab("Fraction of genome altered") + xlab("Group")
...
pdf("/mnt/DATA5/tmp/kev/misc/20210918choBoxplotPregGeno.pdf")
ggplot(data = pregFga, aes(x = group2, y = X2)) + geom_boxplot() + 
  geom_signif(comparisons = list(c("BPRN","BPRN fl/+")), test = "t.test") + 
  ggtitle("Boxplot Fga Pregnancy(Nf1)") + 
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
                     plot.title = element_text(hjust = 0.5)) +
  ylab("Fraction of genome altered") + xlab("Group")
...
### stats for Kathy paper
paperSamples <- c(pregFga$X1, agingFga$X1)
...
allStats$Sample.Name <- nameStripper(allStats$Sample.Name)
paperStats <- allStats[which(allStats$Sample.Name %in% paperSamples),]
mean(paperStats$Mapped.Reads)
mean(paperStats$Mean.Depth)
summary(paperStats$Mean.Depth)
summary(paperStats$Mapped.Reads)
tmpVars <- readxl::read_xls("/mnt/DATA5/tmp/kev/misc/20210903combinedChoVarList.xls")
tmpVars$Sample <- nameStripper(tmpVars$Sample)
tmpVars[which(tmpVars$Sample %in% paperStats$Sample.Name),]
