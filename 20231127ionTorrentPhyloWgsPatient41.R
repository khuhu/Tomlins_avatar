...
listOfTcs2$tc2[which(listOfTcs2$tc2 == "Small")] <- 0.5
listOfTcs2$tc2[which(listOfTcs2$tc2 < 0.5)] <- 0.5
listOfTcs2$tc2 <- as.numeric(listOfTcs2$tc2)
listOfTcs2$sample2 <- paste0(str_remove(listOfTcs2$SAMPLE, "-"), "_20_CCP")
...
### didn't look real
if (length(which(allSeg_filt2$seg.mean2_round == 5))) {
  allSeg_filt2 <- allSeg_filt2[-which(allSeg_filt2$seg.mean2_round == 5), ]
}
...
# write.table(preResCnvFinal, "/mnt/DATA5/tmp/kev/misc/20231127patient41phylowgsCnv.txt", sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)
# 
# write.table(ssmRes_filt, "/mnt/DATA5/tmp/kev/misc/20231127patient41phylowgsSsm.txt", sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)
