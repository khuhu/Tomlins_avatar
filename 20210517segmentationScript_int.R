...
opt_parser = OptionParser(option_list=option_list);1
...
opt$bed <- "/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-76-MG_test1_255_185/bed.txt"
opt$amplicon <- "/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-76-MG_test1_255_185/cnAmplicon_matrix.txt"
opt$gc <- "/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-76-MG_test1_255_185/gcCorrectedCounts_matrix.txt"
opt$normal <- "/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-76-MG_test1_255_185/normals.txt"
opt$out <- "/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-76-MG_test1_255_185/segResults.txt"
opt$param <- "/mnt/DATA6/mouseData/copynumber/Auto_user_AUS5-76-MG_test1_255_185/param.txt"
...
ampSeg <- function(mouseAmplicons, mouseBedFile, outDir, minAmp){ 
  ...
  segResults <- foreach(i=allMouseCalls,
                        .combine = 'rbind', .packages = c('DNAcopy')) %dopar% {
                          ...
                          # trying new set of params - default 
...
for (i in which(!is.na(matchingTc))) {
  ...
  if (segRes2$seg.mean[i] > 2^0.2) {
    ...
    print(mouseAmplicons[[matchingTcName[i]]][tmpIdx])
...
write.table(mouseAmplicons, paste0(outDir, "mouseAmplicons.txt"), sep = "\t", quote = FALSE,
            col.names = TRUE, row.names = FALSE)
...
### create segPlots using own code
...
for (i in unique(segZfilt$ID)) {
  ...
  freqPlot_cn(testDf_cn, main = paste(i, "MousePanel tc: ", matchingTc))
  dev.off()
