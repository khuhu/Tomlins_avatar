...
args <- c("/home/kevhu/tmp3/amplicon.GCinput.txt", "/home/kevhu/tmp3/sampleInfo.input.txt", "/home/kevhu/tmp3/amplicon.combinedCoverage.input.txt",
          "--min-amplicons-per-gene=3")

args <- c("/mnt/DATA4/kevhu/tmp/amplicon.GCinput.txt", "/mnt/DATA4/kevhu/tmp/sampleInfo.input.txt", "/mnt/DATA4/kevhu/tmp/amplicon.combinedCoverage.input.txt",
          "--min-amplicons-per-gene=3")
...
args <- c("/mnt/DATA4/kevhu/tmp4/amplicon.GCinput.txt", "/mnt/DATA4/kevhu/tmp4/sampleInfo.input.txt", "/mnt/DATA4/kevhu/tmp4/amplicon.combinedCoverage.input.txt",
          "--min-amplicons-per-gene=3")

args <- c("/mnt/DATA4/kevhu/tmp5/amplicon.GCinput.txt", "/mnt/DATA4/kevhu/tmp5/sampleInfo.input.txt", "/mnt/DATA4/kevhu/tmp5/amplicon.combinedCoverage.input.txt",
          "--min-amplicons-per-gene=3")
...
for(i in seq_along(df$Gene)){
  if(grepl("Pool1", df$Gene[i])){
    df$PoolNumber[i] <- "1" 
  }
  if(grepl("Pool2",  df$Gene[i])){
    df$PoolNumber[i] <- "2" 
  }
  if(grepl("Pool3",  df$Gene[i])){
    df$PoolNumber[i] <- "3" 
  }
  if(grepl("Pool4",  df$Gene[i])){
    df$PoolNumber[i] <- "4" 
  }
}
...
#  geneMatrixQuant <- NULL
#  geneMatches <- grep(geneList[i], df$Gene)
  #geneMatrixQuant <- data.frame(nrow = length(geneMatches))
  #geneMatrixQuant <- data.frame()
#  genesubset <- df[geneMatches,]
...
#                     nrow = abs(dim(Pool2)[1] - dim(Pool1)[1]), ncol = dim(Pool1)[2])
#  colnames(dummyMat) <- colnames(Pool1)
#  Pool1length <- nrow(Pool1)
#  Pool2length <- nrow(Pool2)
#  if(Pool1length > Pool2length){
#    Pool2 <- rbind(Pool2, dummyMat)
#  }
#  if(Pool2length > Pool1length){
#    Pool1 <- rbind(Pool1, dummyMat)
#  }
#  for(x in allNames){
#    j <- which(dfColNames == x)
#    combMatPool1.names <- Pool1$AmpliconId
#    combMatPool2.names <- Pool2$AmpliconId
#    combMat <- cbind(Pool1[[x]], Pool2[[x]])
#    combMat.normed <- normalize.quantiles(combMat, copy = FALSE)
#    pool1.normed <- data.frame(ampliconNames = combMatPool1.names, x = as.numeric(combMat.normed[,1]))
#    pool2.normed <- data.frame(ampliconNames = combMatPool2.names, x = as.numeric(combMat.normed[,2]))
#    pool1.normed <- na.omit(pool1.normed)
#    pool2.normed <- na.omit(pool2.normed)
#    pool.combined <- rbind(pool1.normed, pool2.normed)
#    pool.combined$idx <- as.numeric(gsub("[A-Z]*[[:punct:]]","", pool.combined$ampliconNames))
#    pool.combined <- pool.combined[order(pool.combined$idx),]
#    pool.combined$idx <- NULL
#    df[geneMatches,j]
#    pool.combined[,2]
#    df[geneMatches,j] <- pool.combined[,2]
#    #geneMatrixQuant <- cbind(geneMatrixQuant,pool.combined[,2])
#  }
#  #colnames(geneMatrixQuant) <- sampNames
#  #df[geneMatches,] <- geneMatrixQuant
#}
...
###log-likelihood rather than the original likelihood function.
...
for (samp in allNames) {
  ...
  if (variantData) pboth(samp, cex.axis=0.5, ylim=c(-2,4), analysisResults=analysisResults) else psamp(samp, cex.axis=0.25, ylim=c(-4,4), analysisResults=analysisResults);
  # psamp(samp, cex.axis=0.5, ylim=c(-2,4), analysisResults=analysisResults);
  #dev.off();
  #png(paste("out.", samp, ".png", sep=""), width=7, height=5, units="in", res=400);
  #if (variantData) pboth(samp, cex.axis=0.5, ylim=c(-2,4), analysisResults=analysisResults) else psamp(samp, cex.axis=0.5, ylim=c(-2,4), analysisResults=analysisResults);
  # psamp(samp, cex.axis=0.5, ylim=c(-2,4), analysisResults=analysisResults);
  dev.off();
...
for (samp in allNames) {
  setwd("/mnt/DATA4/kevhu/tmp3/")
  pdf(file = paste("newsamp.",samp, ".pdf",sep = ""),onefile = TRUE, useDingbats = FALSE, width = 30.00)
  newpsamp(samp)
  dev.off()
  #rm(testggplot)
}
