...
args <- c("/mnt/DATA4/kevhu/andi/20200304ctc/n2pool/amplicon.GCinput.txt",
          "/mnt/DATA4/kevhu/andi/20200304ctc/n2pool/sampleInfo.input.txt",
          "/mnt/DATA4/kevhu/andi/20200304ctc/n2pool/amplicon.combinedCoverage.input.txt",
          "--min-amplicons-per-gene=3")
...
test <- read.table(readCountsFilename, header = TRUE, stringsAsFactors = FALSE, sep = ",")
...
for (samp in allNames) {
  ...
  setwd("/mnt/DATA4/kevhu/andi/20200304ctc/tmp/")
  
  
  pdf(paste("out.", samp, ".pdf", sep=""), width=7 * 1.25, height=5 * 1.25, useDingbats=FALSE);
  if (variantData) pboth(samp, cex.axis=0.5, ylim=c(-2,4), analysisResults=analysisResults) else psamp(samp, cex.axis=0.25, ylim=c(-2,4), analysisResults=analysisResults);
...
write.table(geneEst,"/mnt/DATA4/kevhu/PanGuPanelAnalysis/20181205cnGeneRatioNoPool4Amp93Gene.txt", quote = FALSE, col.names = TRUE, 
            row.names = FALSE, sep = "\t")
