...
listOfVcfReport <- c("Auto_user_SAT-93-PR351_ccp_Pr366_ocpv1_168_231",
                     "Auto_user_AUS5-23-PR351-437PanGU_2ndrun_196_070")
listOfVcfReport <- paste0(listOfVcfReport, "_anno.txt")


listOfSamples <- c("PR351","PR352", "PR353", "PR354",
                   "PR355", "PR356", "PR357")

finalVariantList <- read.table("/mnt/DATA5/tmp/kev/misc/Final variant matrix_Pt1.csv",
                               sep = ",", header = TRUE)
...
listOfTcs <- read.table("/mnt/DATA5/tmp/kev/misc/sriSimpa_tumor_fractions.csv",
                        sep = ",", header = TRUE)
listOfTcs <- listOfTcs[which(listOfTcs$PATIENT == "Pt1"), ]
listOfTcs2 <- listOfTcs[, c("SAMPLE", "TUMOR_CONTENT_FINAL")]
...
listOfTcs2$sample2 <- str_remove(listOfTcs2$SAMPLE, "-")
...
phyloWgs_ssm_filt <- function(listOfVcfReport, listOfSamples, listOfVars){
  ...
  for (i in seq_along(listOfSamples)) {
    ...
    ### change depending on report - otherwise it is harmless
...
expectedLogRCalc(0.6, 0, 1, 2)
expectedLogRCalc(0.6, 2, 2, 2)
expectedLogRCalc(0.6, 2, 3, 2)
expectedIntCalc(0.6, 0.8036, 2)
expectedIntCalc(0.6, -0.5759, 2)

allSeg_filt$seg.mean2 <- (2^(allSeg_filt$seg.mean * 2) / vectorTc)
allSeg_filt$tmp <- (2^(allSeg_filt$seg.mean) * 2) / vectorTc
...
# allSeg_filt2 <- allSeg_filt2[-which(allSeg_filt2$seg.mean2_round > 5),]
...
### didn't look real
if (length(which(allSeg_filt2$seg.mean2_round == 4))) {
  allSeg_filt2 <- allSeg_filt2[-which(allSeg_filt2$seg.mean2_round == 4), ]
}
...
for (i in listOfReport) {
  ...
  if (i == "Auto_user_SAT-93-PR351_ccp_Pr366_ocpv1_168_231") {
...
allSampsDisjoinReads$a[which(is.na(allSampsDisjoinReads$a))] <- allSampsDisjoinReads$d[which(is.na(allSampsDisjoinReads$a))]
...
i <- unique(allSampsDisjoinReads$str)[1]

### this is fine
preResCnv <- NULL
preResCnvCoords <- NULL
i <- unique(allSampsDisjoinReads$str)[62]
for (i in unique(allSampsDisjoinReads$str)) {
  ...
  ### same with variants with missing data - not available - median reads of those that do appear
...
preResCnv$ssms[which(preResCnv$ssm == "s0")] <- "s0,1,1"
preResCnv$ssms[which(preResCnv$ssm == "s3")] <- "s3,1,1"
### first time it shows a ssm predates a cnv

preResCnvFinal <- preResCnv[, c("cnv", "a", "d", "ssms", "physical_cnvs")]


# write.table(preResCnvFinal, "/mnt/DATA5/tmp/kev/misc/20231127patient1phylowgsCnv.txt", sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)
# 
# write.table(ssmRes_filt, "/mnt/DATA5/tmp/kev/misc/20231127patient1phylowgsSsm.txt", sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)
