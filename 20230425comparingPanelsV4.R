### previous version /home/kevhu/scripts/20220704comparingPanelsV3.R
### biggest change here is to do comparisons of the best 2n, 3n, 4n for each sample
### when doing the stats comparison split by good and bad samples for confusion matrix

### for now I will just do the listed ploidy I have previously; after meeting with scott expand
...
allNoshadSeg <-  read.table("/mnt/DATA5/tmp/kev/qcAllMousePaper/segResultsFilt.txt", sep = "\t",
...
allAscatSegs_filt <- allAscatSegs[which(allAscatSegs$sample %in% allSampsCorDf2_bestV3$string5),]
...
goodNoshad <- noshadAneploidy[which(rownames(noshadAneploidy) %in% goodSamps), 1:19]
goodAscat <- ascatAneploidy[which(rownames(noshadAneploidy) %in% goodSamps), 1:19]
goodNoshadVec <- as.vector(goodNoshad)
goodAscatVec <- as.vector(goodAscat)
goodNoshadVec<- str_replace_all(goodNoshadVec, "sub", "none")
goodNoshadVec <- factor(goodNoshadVec, levels = c("none", "gain", "loss"))
goodAscatVec <- factor(goodAscatVec, levels = c("none", "gain", "loss"))
goodConMat <- confusionMatrix(goodNoshadVec, goodAscatVec)


badSamps <- sampleQcDf$sample[which(sampleQcDf$qc == "bad")]
badNoshad <- noshadAneploidy[which(rownames(noshadAneploidy) %in% badSamps), 1:19]
badAscat <- ascatAneploidy[which(rownames(noshadAneploidy) %in% badSamps), 1:19]
badNoshadVec <- as.vector(badNoshad)
badAscatVec <- as.vector(badAscat)
badNoshadVec <- str_replace_all(badNoshadVec, "sub", "none")
badNoshadVec <- factor(badNoshadVec, levels = c("none", "gain", "loss"))
badAscatVec <- factor(badAscatVec, levels = c("none", "gain", "loss"))
badConMat <- confusionMatrix(badNoshadVec, badAscatVec)
...
# goodConByChr <- list()
# for (i in 1:19) {
#   goodNoshadVec <- as.vector(goodNoshad[,i])
#   goodAscatVec <- as.vector(goodAscat[,i])
#   goodNoshadVec <- str_replace_all(goodNoshadVec, "sub", "none")
#   goodNoshadVec <- factor(goodNoshadVec, levels = c("none", "gain", "loss"))
#   goodAscatVec <- factor(goodAscatVec, levels = c("none", "gain", "loss"))
#   tmpConMat <- confusionMatrix(goodNoshadVec, goodAscatVec)
#   goodConByChr[[i]] <- tmpConMat
# }


### doing this with sub clonal changes - ascat matrix has no change
###
###


noshadAneploidySub <- matrix(0, nrow = length(unique(panelV3$sample)), ncol = length(unique(allNoshadSeg_filt$chr)))
rownames(noshadAneploidySub) <- unique(panelV3$sample)
colnames(noshadAneploidySub) <- unique(allNoshadSeg_filt$chr)
...
goodNoshadSub <- noshadAneploidySub[which(rownames(noshadAneploidySub) %in% goodSamps), 1:19]
goodNoshadSubVec <- as.vector(goodNoshadSub)
goodNoshadSubVec <- factor(goodNoshadSubVec, levels = c("none", "gain", "loss"))
goodAscatVec <- factor(goodAscatVec, levels = c("none", "gain", "loss"))
goodConMat <- confusionMatrix(goodNoshadSubVec, goodAscatVec)

badNoshadSub <- noshadAneploidySub[which(rownames(noshadAneploidySub) %in% badSamps), 1:19]
badNoshadSubVec <- as.vector(badNoshadSub)
badAscatVec <- as.vector(badAscat)
badNoshadSubVec <- factor(badNoshadSubVec, levels = c("none", "gain", "loss"))
badAscatVec <- factor(badAscatVec, levels = c("none", "gain", "loss"))
badConMat <- confusionMatrix(badNoshadSubVec, badAscatVec)


### double check the split in good and bad samples
### then look at if the loss in sensitivity and specificity is chromosome specific
### afterwards maybe certain areas are bad by no gapped segmentation
...
goodChrom <- data.frame()
badChrom <- data.frame()
allChrom <- data.frame()
for (i in 1:19) {
  goodNoshadSub <- noshadAneploidySub[which(rownames(noshadAneploidySub) %in% goodSamps), i]
  goodNoshadSubVec <- as.vector(goodNoshadSub)
  goodNoshadSubVec <- factor(goodNoshadSubVec, levels = c("none", "gain", "loss"))
  goodAscatVec <- factor(goodAscat[,i], levels = c("none", "gain", "loss"))
  goodConMat <- confusionMatrix(goodNoshadSubVec, reference = goodAscatVec)
  
  badNoshadSub <- noshadAneploidySub[which(rownames(noshadAneploidySub) %in% badSamps), 1]
  badNoshadSubVec <- as.vector(badNoshadSub)
  badAscatVec <- as.vector(badAscat)
  badNoshadSubVec <- factor(badNoshadSubVec, levels = c("none", "gain", "loss"))
  badAscatVec <- factor(badAscat[, i], levels = c("none", "gain", "loss"))
  badConMat <- confusionMatrix(badNoshadSubVec, reference = badAscatVec)
  
  tmpBadDf <- data.frame("chrom" = rep(i, 3), "sample" = rep("bad", 3), "type"  = rownames(badConMat$byClass), badConMat$byClass)
  badChrom <- rbind(badChrom, tmpBadDf)
  tmpGoodDf <- data.frame("chrom" = rep(i, 3), "sample" = rep("good", 3), "type"  = rownames(goodConMat$byClass), goodConMat$byClass)
  goodChrom <- rbind(goodChrom, tmpGoodDf)
  
  allChrom <- rbind(allChrom, tmpGoodDf, tmpBadDf)
}

rownames(allChrom) <- NULL
rownames(badChrom) <- NULL
rownames(goodChrom) <- NULL
