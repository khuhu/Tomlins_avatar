...
mouse_ploidy <-  readxl::read_xlsx("/mnt/DATA5/tmp/kev/misc/20210921ploidyAssesmentsHgovc.xlsx")
mouse_ploidy <- mouse_ploidy[-which(is.na(mouse_ploidy$IntegerP_2)), c(1,5:7)]
colnames(mouse_ploidy) <- c("Sample", "p0", "P0", "IntegerP")
...
# save(armRes, file = "/mnt/DATA5/tmp/kev/misc/20210916tcgaArmRes5Mb.Robj")
load("/mnt/DATA5/tmp/kev/misc/20210916tcgaArmRes5Mb.Robj")
...
matchingId <- match(tolower(all_segRes$ID), mouse_ploidy$Sample)
# all_segRes$ID[which(is.na(matchingId))]
all_segRes <- all_segRes[-which(is.na(matchingId)),]
matchingId2 <- match(tolower(all_segRes$ID), mouse_ploidy$Sample)
all_segRes$absCn <- round2((2^(all_segRes$seg.mean) * mouse_ploidy$IntegerP[matchingId2])/mouse_ploidy$p0[matchingId2], 0)
all_segRes$absCnCor <- all_segRes$absCn - mouse_ploidy$IntegerP[matchingId2]

# use for raw log2(cnrs)
...
segZfilt_BPRN <- segZscoresFilt_zeroOut(all_segRes_BPRN, abs = TRUE)
...
segZfilt_BPP <- segZscoresFilt_zeroOut(all_segRes_BPP, abs = TRUE)
...
segZfilt_BPN <- segZscoresFilt_zeroOut(all_segRes_BPN, abs = TRUE)
...
segZfilt_PRN <- segZscoresFilt_zeroOut(all_segRes_PRN, abs = TRUE)
...
a <- ggplot(gi_count_BPRN_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("BPRN n = 77") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
b <- ggplot(gi_count_BPP_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("BPP n = 10") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
c <- ggplot(gi_count_BPN_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("BPN n = 7") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
d <- ggplot(gi_count_PRN_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("PRN n = 10") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
# e <- ggplot(gi_count_WGS_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("WGS n = 10") +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
f <- ggplot(gi_count_hgsc_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("hgsc n = 331") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
g <- ggplot(gi_count_brca_df) + geom_boxplot(aes(x = variable, y = value)) + ylim(c(0,70)) + ggtitle("hgsc(brca) n = 33") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

dev.off()
pdf("/mnt/DATA5/tmp/kev/misc/20210920countingGI.pdf", useDingbats = FALSE)
# grid.arrange(a,b,c,d,e,f,g, ncol =3)
grid.arrange(a,b,c,d,f,g, ncol =3)
dev.off()
...
fga_all <- rbind(fga_bpn, fga_bprn, fga_bpp,
                 fga_prn,fga_hgsc,fga_brca)
dev.off()
pdf("/mnt/DATA5/tmp/kev/misc/20210029_fga_plody.pdf", useDingbats = FALSE)
...
dev.off()
pdf("/mnt/DATA5/tmp/kev/misc/20210921_PloidyFreqPlotsAllArms.pdf", height = 9, width = 9, useDingbats = FALSE)
#grid.arrange(a,b,c,d,e,f,g, ncol = 1)
grid.arrange(a,b,c,d,e,f, ncol = 1)
grid.arrange(a,b,c,e,f, ncol = 1)
dev.off()
...
dev.off()
pdf("/mnt/DATA5/tmp/kev/misc/20210920ploidyFreqPlotsAllCnas.pdf", height = 9, width = 9, useDingbats = FALSE)
#grid.arrange(a,b,c,d,e,f,g, ncol = 1)
grid.arrange(a,b,c,d,e,f, ncol = 1)
dev.off()
