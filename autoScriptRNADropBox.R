...
pathToEros <- "/mnt/DATA3/eros_data_dump/";
pathToShinyApp <- "/mnt/DATA4/kevhu/apps/fetchApp/";
listofSequencers <- c(pathToYoda, pathToZues, pathToEros);
...
for(i in seq_along(listOfRNAJsons2)){
  ...
  for(j in seq_along(g$barcodes)){
    ...
    address <- sub("results.json","", listOfRNAJsons2[i])
    ...
    varCheckSetwd <- tryCatch(setwd(dirForCovFiles), error = function(x) return(NULL));
    if(is.null(varCheckSetwd)){
      next()
    }
