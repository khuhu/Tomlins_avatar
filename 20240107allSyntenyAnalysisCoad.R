### copying hgsc pipeline for coad-reanalysis
...
### files are downloaded on brahm for some read it doesn't work on avatar, but I can prepare the files
...
tcga_coad_txn <- GDCprepare(queryTCGA, summarizedExperiment = TRUE,
                          directory = "/mnt/DATA5/tmp/kev/tmpDbs/tcgaBiolinks/")
coad_txn_matrix <- assay(tcga_coad_txn, 'unstranded')

queryTCGA <- GDCquery(project = "TCGA-READ",
                      data.category = 'Transcriptome Profiling',
                      experimental.strategy = 'RNA-Seq',
                      workflow.type = 'STAR - Counts',
                      access = 'Open',
                      sample.type = 'Primary Tumor')
tcga_read_txn <- GDCprepare(queryTCGA, summarizedExperiment = TRUE,
                            directory = "/mnt/DATA5/tmp/kev/tmpDbs/tcgaBiolinks/")
read_txn_matrix <- assay(tcga_read_txn, 'unstranded')

### there are normal samples remove after

library(biomaRt)
library(gprofiler2)
tmpDf <- rownames(coad_txn_matrix)
tmpDf <- gsub('\\..+$', '', tmpDf)
...
coad_txn_matrix2 <- coad_txn_matrix
rownames(coad_txn_matrix2) <- gsub('\\..+$', '', rownames(coad_txn_matrix2))
coad_txn_matrix2 <- coad_txn_matrix2[which(rownames(coad_txn_matrix2) %in% symbol2$ensembl_gene_id), ]
rownames(coad_txn_matrix2) <- symbol2$hgnc_symbol[match(rownames(coad_txn_matrix2), symbol2$ensembl_gene_id)]
coad_txn_matrix2 <- coad_txn_matrix2[-which(duplicated(rownames(coad_txn_matrix2))), ] 

tmpDf <- rownames(read_txn_matrix)
tmpDf <- gsub('\\..+$', '', tmpDf)
mart <- useDataset("hsapiens_gene_ensembl", useMart("ensembl"))
genes <- tmpDf
symbol <- getBM(filters = "ensembl_gene_id",
                ...
                mart = mart)
symbol2 <- symbol[-which(symbol$hgnc_symbol == ""),]
read_txn_matrix2 <- read_txn_matrix
rownames(read_txn_matrix2) <- gsub('\\..+$', '', rownames(read_txn_matrix2))
read_txn_matrix2 <- read_txn_matrix2[which(rownames(read_txn_matrix2) %in% symbol2$ensembl_gene_id), ]
rownames(read_txn_matrix2) <- symbol2$hgnc_symbol[match(rownames(read_txn_matrix2), symbol2$ensembl_gene_id)]
read_txn_matrix2 <- read_txn_matrix2[-which(duplicated(rownames(read_txn_matrix2))), ] 
read_txn_matrix2 <- read_txn_matrix2[match(rownames(coad_txn_matrix2), rownames(read_txn_matrix2)), ]

coadread_txn_matrix2 <- cbind(coad_txn_matrix2, read_txn_matrix2)
### gets rid of the same sample measured twice
coadread_txn_matrix2_t <- coadread_txn_matrix2[, grep("\\-01A", colnames(coadread_txn_matrix2))]
...
combinedRsemRawMat2 <- coadread_txn_matrix2_t
colnames(combinedRsemRawMat2) <-  gsub("(^.*?-.{3}?)-.*", "\\1", colnames(combinedRsemRawMat2))
...
### using xena browser designations
tmpRsemColnames <- gsub("\\-01A", "", colnames(combinedRsemRawMat2))
clinical_coad <- read.table("/mnt/DATA5/tmp/kev/tmpDbs/xena/survivalCOADREAD_survival.txt",
                            sep = "\t", stringsAsFactors = FALSE, header = TRUE)
clinical_coad2 <- clinical_coad[match(tmpRsemColnames, clinical_coad$X_PATIENT),]
colnames(clinical_coad2)[1] <- "submitter_id2"
colnames(clinical_coad2) <- str_replace_all(colnames(clinical_coad2), "\\.", "_")
clinical_coad2$PFI_status <- ifelse(clinical_coad2$PFI == 1, TRUE, FALSE)

### doing it by arm status
coadArmStatus <- cbind(hg19ArmLocations$str, broadBySampleGisticCoad2[, 4:ncol(broadBySampleGisticCoad2)])
colnames(coadArmStatus)[1] <- "Chromosome Arm"
colnames(coadArmStatus)[2:ncol(coadArmStatus)] <- gsub("(^.*?-.{3}?)-.*", "\\1", colnames(coadArmStatus)[2:ncol(coadArmStatus)])
colnames(coadArmStatus)[2:ncol(coadArmStatus)] <- str_replace_all(colnames(coadArmStatus)[2:ncol(coadArmStatus)], "01A", "01")
...
for (i in 1:nrow(coadArmStatusMat)) {
  tmpGain <- colnames(coadArmStatusMat)[which(coadArmStatusMat[i, ] > 0.2)]
  tmpLoss <- colnames(coadArmStatusMat)[which(coadArmStatusMat[i, ] < -0.2)]
...
for (i in seq_along(chromGainStatus)) {
  ...
  tmpVector <- ifelse(clinical_coad2$submitter_id2 %in% tmpNames, "Yes", "No")
  chromGainStatusDf <- cbind(chromGainStatusDf, tmpVector)
}

colnames(chromGainStatusDf) <- paste0("gain_", rownames(coadArmStatusMat))
...
colnames(chromLossStatusDf) <- paste0("loss_", rownames(coadArmStatusMat))

clinical_coad_arm <- cbind(clinical_coad2[, c("submitter_id2", "X_PATIENT", "OS", "OS_time")], chromGainStatusDf, chromLossStatusDf)
# clinical_coad_arm <- cbind(clinical_coad2[, c("submitter_id2", "X_PATIENT", "DSS", "DSS_time")], chromGainStatusDf, chromLossStatusDf)
# clinical_coad_arm <- cbind(clinical_coad2[, c("submitter_id2", "X_PATIENT", "PFI", "PFI_time")], chromGainStatusDf, chromLossStatusDf)


survivalarmsDf_arm <- NULL
for (i in 5:ncol(clinical_coad_arm)) {
  tmpFit <- survdiff(Surv(OS_time, OS) ~ clinical_coad_arm[,i], data = clinical_coad_arm)
  tmpPVal <- pchisq(tmpFit$chisq, length(tmpFit$n)-1, lower.tail = FALSE)
  survivalarmsDf_arm <- rbind(survivalarmsDf_arm, data.frame("var" = colnames(clinical_coad_arm)[i], "pval" = tmpPVal))
}


# survivalarmsDf_arm <- NULL
# for (i in 8:ncol(clinical_coad_arm)) {
#   tmpFit <- survdiff(Surv(DSS_time, DSS) ~ clinical_coad_arm[,i], data = clinical_coad_arm)
#   tmpPVal <- pchisq(tmpFit$chisq, length(tmpFit$n)-1, lower.tail = FALSE)
#   survivalarmsDf_arm <- rbind(survivalarmsDf_arm, data.frame("var" = colnames(clinical_coad_arm)[i], "pval" = tmpPVal))
# }
# 
...
# survivalarmsDf_arm <- NULL
# for (i in 8:ncol(clinical_coad_arm)) {
#   tmpFit <- survdiff(Surv(PFI_time, PFI) ~ clinical_coad_arm[,i], data = clinical_coad_arm)
#   tmpPVal <- pchisq(tmpFit$chisq, length(tmpFit$n)-1, lower.tail = FALSE)
#   survivalarmsDf_arm <- rbind(survivalarmsDf_arm, data.frame("var" = colnames(clinical_coad_arm)[i], "pval" = tmpPVal))
# }
...
clinical_coad_block <- cbind(clinical_coad2[, c("submitter_id2", "X_PATIENT", "OS", "OS_time")], blockStatusGainDf, blockStatusLossDf)
# clinical_coad_block <- cbind(clinical_coad2[, c("submitter_id2", "X_PATIENT", "DSS", "DSS_time")], blockStatusGainDf, blockStatusLossDf)
# clinical_coad_block <- cbind(clinical_coad2[, c("submitter_id2", "X_PATIENT", "PFI", "PFI_time")], blockStatusGainDf, blockStatusLossDf)


survivalBlocksDf_block <- NULL
for (i in 5:ncol(clinical_coad_block)) {
  tmpFit <- survdiff(Surv(OS_time, OS) ~ clinical_coad_block[,i], data = clinical_coad_block)
  tmpPVal <- pchisq(tmpFit$chisq, length(tmpFit$n)-1, lower.tail = FALSE)
  survivalBlocksDf_block <- rbind(survivalBlocksDf_block, data.frame("var" = colnames(clinical_coad_block)[i], "pval" = tmpPVal))
}


### similar to above, but instead of edgeR use DEseq2

combinedRsemRawMat2 <- combinedRsemRawMat2[, -which(duplicated(colnames(combinedRsemRawMat2)))]
...
# write.table(diffArmAmpDeSeqDf, "/mnt/DATA5/tmp/kev/misc/20240107diffArmAmpDeSeq.txt",sep = "\t",
...
# write.table(diffArmDelDeSeqDf, "/mnt/DATA5/tmp/kev/misc/20240107diffArmDelDeSeq.txt",sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)



diffArmAmpDeSeqDf$blockStripped <- str_remove(diffArmAmpDeSeqDf$block, "_amp_")
diffArmAmpDeSeqDf$h_chr <- adenoCar_arm_allSynTable_amp_good$h_chr[match(diffArmAmpDeSeqDf$blockStripped,
                                                                         adenoCar_arm_allSynTable_amp_good$block)]

diffArmDelDeSeqDf$blockStripped <- str_remove(diffArmDelDeSeqDf$block, "_loss_")
diffArmDelDeSeqDf$h_chr <- adenoCar_arm_allSynTable_del_good$h_chr[match(diffArmDelDeSeqDf$blockStripped,
                                                                         adenoCar_arm_allSynTable_del_good$block)]


allDiffArmDeSeqDf <- rbind(diffArmDelDeSeqDf, diffArmAmpDeSeqDf)
i <- unique(allDiffArmDeSeqDf$block)[1]
...
### depmap cross comparison 

depmapTableRnai <- depmap::depmap_rnai()
depmapTableRnai_coadread <- depmapTableRnai[grep("COLO", depmapTableRnai$cell_line),]
depmapTableCrispr <- depmap::depmap_crispr()
depmapTableCrispr_coadread <- depmapTableCrispr[grep("COLO", depmapTableCrispr$cell_line),]
depmapRnai_nom <- depmapTableRnai_coadread[which(depmapTableRnai_coadread$gene_name %in% nominatedGenesDf$genes), ]
depmapCripspr_nom <- depmapTableCrispr_coadread[which(depmapTableCrispr_coadread$gene_name %in% nominatedGenesDf$genes), ]
...
pdf("/mnt/DATA5/tmp/kev/misc/20230110coadreadTcgaFreqplot.pdf", width = 10, height = 5)
e
dev.off()

pdf("/mnt/DATA5/tmp/kev/misc/20230110coadreadMouseAdenomaFreqplot.pdf", width = 10, height = 5)
f
dev.off()

pdf("/mnt/DATA5/tmp/kev/misc/20230110coadreadMouseAdenoCarFreqplot.pdf", width = 10, height = 5)
g
dev.off()
...
pdf(file = "/mnt/DATA5/tmp/kev/misc/20240110coadreadTcgaFgaComp.pdf", useDingbats = TRUE,
    width = 7, height = 4)
...
### to get p-values, since our sample set is < 50 samples shapiro wilkes instead of KS test
library(DataScienceR)
fgaPairwiseResults <- pairwise_ks_test(allFga$fga, allFga$Cancer, n_min = 10, alternative = "two.sided")
View(fgaPairwiseResults)

fgaPairwiseResults <- fgaPairwiseResults[, c("mm_coadread", "mm_adeno")]
...
# pdf(file = "/mnt/DATA5/tmp/kev/misc/20240110coadreadFractionSyntenyReccurentSign.pdf", useDingbats = TRUE,
#     width = 10, height = 5)
ggplot(barplotFractionFiltMouseCoad, aes(fill= h_chr, y= fraction, x = m_chr)) +
  geom_bar(position="stack", stat="identity", color = barplotFractionFiltMouseCoad$color,
           alpha =  barplotFractionFiltMouseCoad$alpha, linewidth = 1.5) +
  scale_fill_manual(values = rep("#A9A9A9", 22)) +
...
pdf(file = "/mnt/DATA5/tmp/kev/misc/20240110coadreadFractionSyntenyConMouseRefPerm.pdf", useDingbats = TRUE,
    width = 10, height = 5)
ggplot(barplotFractionFiltMouseCoad2, aes(fill= h_chr, y= fraction, x = m_chr)) + 
  geom_bar(position="stack", stat="identity", color = barplotFractionFiltMouseCoad2$color,
           alpha =  barplotFractionFiltMouseCoad2$alpha, linewidth = 1.5) + 
