...
preResCnv$ssms[which(preResCnv$ssm == "s8")] <- "s8,1,1"
preResCnv$ssms[which(preResCnv$ssm == "s6")] <- "s6,1,1"
preResCnv$ssms[which(preResCnv$ssm == "s1")] <- "s1,1,1"
### first time it shows a ssm predates a cnv
preResCnv$ssms[which(preResCnv$ssm == "s2")] <- "s2,0,1"

preResCnvFinal <- preResCnv[, c("cnv", "a", "d", "ssms", "physical_cnvs")]


# write.table(preResCnvFinal, "/mnt/DATA5/tmp/kev/misc/20231127patient33phylowgsCnv.txt", sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)
# 
# write.table(ssmRes_filt, "/mnt/DATA5/tmp/kev/misc/20231127patient33phylowgsSsm.txt", sep = "\t",
#             row.names = FALSE, col.names = TRUE, quote = FALSE)
# 
# 
