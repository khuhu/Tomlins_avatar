...
df <- read.xlsx("/mnt/DATA2/share/Users/Kevin/LorenaStuff/New_ID_IE_Final Zygosity Table for Kevin.xlsx", sheetIndex = 3, sheetName = "IE",stringsAsFactors=FALSE)
...
Predom <- which(df$Type == typeLesion[4])
...
Pre.tabe <- dummyTable2[Predom,]

listOfTab <- c("AK.tabe", "IS.tabe", "In.tabe","Pre.tabe")
...
grouping <- c(rep("Actinic Keratosis",maxRows),rep("In Situ",maxRows),rep("Invasive",maxRows),rep("Predom",maxRows))
grouping <- rep(grouping,4)



listOfTypes <- c("Actinic Keratosis CDKN2A Het","In Situ CDKN2A Het","Invasive CDKN2A Het", "Predom CDKN2A Het",
                 "Actinic Keratosis CDKN2A Homo","In Situ CDKN2A Homo","Invasive CDKN2A Homo", "Predom CDKN2A Homo",
                 "Actinic Keratosis TP53 Het","In Situ TP53 Het","Invasive TP53 Het", "Predom TP53 Het", 
                 "Actinic Keratosis TP53 Homo","In Situ TP53 Homo","Invasive TP53 Homo", "Predom TP53 Homo")
...
pdf(file = "/mnt/DATA4/kevhu/LorenaData/20180703IEZygosityPieChart.pdf", useDingbats = FALSE)
ggplot(data = finalMatrix.pie, aes(x = factor(1) , y = Freq,fill = NumMut)) +
  ...
  scale_x_discrete(name = "Predom   Invasive   In Situ     Actinic Keratosis") 
dev.off()
