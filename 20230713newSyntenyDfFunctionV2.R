...
circosFreq2 <- function(m_amp, m_del, tcga_amp, tcga_del, filename = "test",
                        geneVar = "no", ref = "human", empty = FALSE){
...
freqPlotv2 <- function(gainsDf, lossesDf, speciesType = "human", main = "no title"){
  ...
  ggplot(tmpAmpDel_graph2) + geom_vline(xintercept=chromBreakNoX) + geom_hline(yintercept = 0) + 
    ...
    geom_text(data = chromTextdfNoX, aes(x = xpos, y = ypos, label = chrom), size = 2.5) + ggtitle(main)
...
### to only calculate aneuploidy
fgaCalculator_tcga_aneuploidy <- function(gisticArm){
  tmpRes <- NULL
  for (i in unique(gisticArm$sampleID)) {
    tmpDfArm <- gisticArm[which(gisticArm$sampleID == i),]
    armSum <- sum(tmpDfArm$length[which(abs(tmpDfArm$mean) > 0.2)])
    tmpFga <- armSum/sum(tmpDfArm$length)
    tmpRes <- rbind(tmpRes, data.frame("sample" = i, "fga" = tmpFga))
  }
  return(tmpRes)
}
...
fgaCalculator_amp_aneuploidy <- function(ampliconArm){
  tmpRes <- NULL
  for (i in unique(ampliconArm$sampleID)) {
    tmpDfArm <- ampliconArm[which(ampliconArm$sampleID == i),]
    armSum <- sum(tmpDfArm$length[which(abs(tmpDfArm$mean) > 0.2)])
...
# m_amp <- allMouseAneu_coadAdenoCar_amp_bed
# m_del <- allMouseAneu_coadAdenoCar_del_bed
# tcga_amp <- armGisticCoadApc_amp_bed
# tcga_del <- armGisticCoadApc_del_bed
# ref <- "human"
# chromosome <- "h_chr13"

circosFreqSingleChr <- function(m_amp, m_del, tcga_amp, chromosome = "h_chr13",
                                tcga_del, filename = "test", ref = "human"){
  require(circlize)
  ...
  allSynTable <- allSynTable[which(allSynTable$h_chr == chromosome), ]
  
  
  if (ref == "human") {
    mouse_bed <- mouse_bed[which(human_bed$chr %in% unique(allSynTable$h_chr)), ]
    human_bed <- human_bed[which(human_bed$chr %in% unique(allSynTable$h_chr)), ]
  } else if(ref == "mouse"){
    mouse_bed <- mouse_bed[which(mouse_bed$chr %in% unique(allSynTable$m_chr)), ]
    human_bed <- human_bed[which(mouse_bed$chr %in% unique(allSynTable$m_chr)), ]
  }
  


  cyto_interest <- c(unique(allSynTable$h_chr), unique(allSynTable$m_chr))
  cyto_combined_red <- cyto_hg19mm10[which(cyto_hg19mm10$V1 %in% cyto_interest),]
  ...
  mouse_cn <- mouse_cn[which(mouse_cn$chr %in% unique(allSynTable$m_chr)), ]
  human_cn <- human_cn[which(human_cn$chr %in% unique(allSynTable$h_chr)), ]
